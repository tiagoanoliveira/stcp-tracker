/**
 * LineDetailApp.js - Pagina de detalhe de uma linha STCP
 */
...
const BUS_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="5" width="18" height="13" rx="2"/><path d="M3 10h18"/><path d="M8 18v2M16 18v2"/><circle cx="8" cy="14.5" r="1"/><circle cx="16" cy="14.5" r="1"/></svg>';
...
async function init() {
  if (!routeId) { showError('Linha nao encontrada.'); return; }
  try {
...
async function loadStops() {
  ...
  try {
    ...
    showError('Nao foi possivel carregar as paragens da linha ' + routeNumber + '.');
  }
}
...
function renderStops(stops) {
  ...
  stopsEl.innerHTML = stops.map((s, i) => {
    ...
    return '<li class="' + cls + '">' +
      '<div class="tl" style="--line-color:' + bg + '">' + dotHtml + '</div>' +
